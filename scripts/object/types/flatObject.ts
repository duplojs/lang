	...
	? (
		| (
			Exclude<GenericObject, readonly any[]> extends infer InferredObject extends object
				? DCommon.IsEqual<InferredObject, never> extends true
					? never
					: {
		...
		| (
			GenericObject extends DCommon.AnyTuple
				? DTuple.HasSpread<GenericObject> extends true
					? never
		...
		| (
			Exclude<
				Extract<GenericObject, readonly any[]>,
				DCommon.AnyTuple
			> extends infer InferredArray extends readonly any[]
				? DCommon.IsEqual<InferredArray, never> extends true
					? never
		...
		? DCommon.IsEqual<GenericPath, ""> extends true
