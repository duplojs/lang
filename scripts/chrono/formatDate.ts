...
export function formatDate(
	...args:
		| [formatString: string, timezone: Timezone]
		| [date: TheDate | SerializedTheDate, formatString: string, timezone: Timezone]
) {
	if (args.length === 2) {
		const [formatString, timezone] = args;

		return (date: TheDate | SerializedTheDate) => formatDate(date, formatString, timezone);
	}

	const [date, formatString, timezone] = args;
	...
	const tokens: Record<FormatToken, string> = {
		...
		mm: parts.minute,
		ss: parts.second,
		SSS: nativeDate.getMilliseconds().toString(),
		ZZ: timezone,
	};
